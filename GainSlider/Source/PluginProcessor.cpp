...
void GainSliderAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    // Main processing loop that handles all alterations to the input signal
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        auto* channelData = buffer.getWritePointer (channel);

        // Alter the gain of the input signal according to the current knob value
        for (int sample = 0; sample < buffer.getNumSamples(); ++sample) {
            channelData[sample] *= juce::Decibels::decibelsToGain(gainKnobVal);
        }
    }
}
